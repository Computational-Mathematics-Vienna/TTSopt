...
function [xbest,fbest] = IMATRS(fun,x,low,upp,st,tune)
global finfo

finfo.low=low; 
finfo.upp=upp;
finfo.error='';
finfo.nfmax=st.nfmax;
finfo.done=0;
finfo.nf = 1;
finfo.prt=st.prt;
finfo.cpu0=cputime;
finfo.secmax=st.secmax;
...
if length(finfo.upp)~=n
  nlow=n,nupp=length(finfo.upp)
...
if ~isfield(tune,'Deltainit'), tune.Deltainit = 50;  end
...
M = eye(n);
ordering = 'ascend';
xf=Inf*ones(finfo.nfmax,n+1);
eta         = 1.5;
Phalton     = haltonset(n);
ihalton     = 7;
allones     = 0;
x = max(finfo.low,min(finfo.upp,x));
xbest = x;
[f] = fun(x); 
fbest=f;
...
nx=size(x,1);
xf(finfo.nf,1:nx)=x';
xf(finfo.nf,nx+1)=fbest;
% check stopping test
a = ones(lambda,1); succ = zeros(1,n); D=eye(n,n); 
it = 0; aold=inf; finfo.prt=0; prt = finfo.prt;
while 1
       it = it+1;
       x=xbest; 
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       if finfo.done; return; end 
        ...
        if (norm(xbest-x) <= 0) && (max(a) == 1) && (aold == 1)
            ...
            while iexit == 0
               ...
               if eta >= 0.5*(norm(finfo.upp-finfo.low)./2)
                   iexit = 1;
               end
            end
        end
       ...
       if mu>=2
         ...
         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          if prt>=2
             fprintf(['start of ',num2str(it),'th selection\n'])
          end
         ...
         for m = 1:mu; 
          ...
          Parent.Ps=sum_z;
           if prt>=2
             fprintf(['end of ',num2str(it),'th selection\n'])
          end
          % update M
       ...
       okTR=(((mu<=1&&~good)||(mu>1&&~good&&norm(d)==0))&&finfo.nf>=n+1); 
