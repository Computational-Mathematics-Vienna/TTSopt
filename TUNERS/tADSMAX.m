...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% tADSMAX generates necessary data for doing tuning the parameters
% of ADSMAX by a mixed integer solver
%
% List of tuning parameters:
%
% mu: mu is the initial percentage change in components
% nstep: max number of times to double or decrease step size
%
% solverParams = struct('mu',0.0001, 'nstep',0)


function tuneinfo = tADSMAX
  ...
  x(1).Values.upp = 1;
  x(1).Current = 0.0001;
  x(1).Type = 'continuous'; % mu (continuous)
  x(2).Values.low = 1;
  x(2).Values.upp = 100;
  x(2).Current = 0;
  x(2).Type = 'integer'; % nstep (integer)
