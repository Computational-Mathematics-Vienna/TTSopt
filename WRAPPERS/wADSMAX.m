...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wADSMAX(prob, func, options)
%
% wrapper for the ADSMAX solver
%
%
% ARGUMENTS
% ---------
...
%
%     .mu (float): 0.0001
%       mu is the initial percentage change in components
%
%     .nstep (int): 0
%       max number of times to double or decrease step size
%
%
% RETURNS
% -------
...
function output = wADSMAX(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('mu',0.0001, 'nstep',0);
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['ADSMAX',num2str(TuneCase)];
      else, solname = 'ADSMAX';
      end
      ...
      % call solver
      try
        output.solverInfo = adsmax(@(x)funf(x)*-1, x, stopit, params);
