...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wMADFO(prob, func, options)
%
% wrapper for the MADFO solver
%
%
% ARGUMENTS
% ---------
...
%       lambda is the sample size         lambda = lambdaCoeff1 +
%       floor(lambdaCoeff2*log(dim))
%
%     .lambdaCoeff2 (int): 3
%       lambda coefficient 2
%       lambda is the sample size         lambda = lambdaCoeff1 +
%       floor(lambdaCoeff2*log(dim))
%
%     .muCoeff (int): 2
%       mu coefficient
%
%     .gammaE_coeff1 (float): 9.0
%       coefficient 1 for parameters for expanding step size
%
%     .gammaE_coeff2 (float): 9.0
%       coefficient 2 for parameters for expanding step size
%
%     .E_coeff1 (float): 100000.0
%       coefficient for maximum iterations of extrapolation step
%
%     .E_coeff2 (float): 100000.0
%       coefficient for maximum iterations of extrapolation step
%
%     .sigmainit (int): 1
%       initial step size
...
%
%     .sigmamax_coeff1 (float): 0.5
%       coefficient 1 for maximum value for sigma
%
%     .sigmamax_coeff2 (float): 0.5
%       coefficient 2 for maximum value for sigma
%
%     .kappa (int): 5
...
function output = wMADFO(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('lambdaCoeff1',4, 'lambdaCoeff2',3,...
    'muCoeff',2, 'gammaE_coeff1',9.0, 'gammaE_coeff2',9.0,...
    'E_coeff1',100000.0, 'E_coeff2',100000.0, 'sigmainit',1,...
    'gamma',1e-12, 'sigmamin',0.01, 'sigmamax_coeff1',0.5,...
    'sigmamax_coeff2',0.5, 'kappa',5, 'N',10);
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['MADFO',num2str(TuneCase)];
      else, solname = 'MADFO';
      end
      ...
      if TuneCase == -1
         ...
         tune.mu = floor(tune.lambda/tune.muCoeff);
