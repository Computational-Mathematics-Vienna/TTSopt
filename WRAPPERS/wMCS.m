...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wMCS(prob, func, options)
%
% wrapper for the MCS solver
%
%
% ARGUMENTS
% ---------
...
%
%     .iinit (int): 1
%       parameter defining the initialization list
%       0: corners and midpoint (default for finite u, v) 1: safeguarded
%       version (default otherwise) 2: 5u/6 + v/6, u/6 + 5v/6 and
%       midpoint
%
%     .local (int): 50
%       maximal number of steps in local search
%       0 means no local search
%
%     .gamma (float): 1e-10
%       stopping criterion for local search
%       stops if abs(g)"*max(abs(x), abs(xold)) < gamma*(f0-f)
%
%
% RETURNS
% -------
...
function output = wMCS(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('smax',50, 'iinit',1, 'local',50,...
    'gamma',1e-10);
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['MCS',num2str(TuneCase)];
      else, solname = 'MCS';
      end
      ...
      % adds paths
      eval(['addpath ',SOLpath,'/MCS']);
      eval(['addpath ',SOLpath,'/MCS/gls2']);
      eval(['addpath ',SOLpath,'/MCS/minq2']);
