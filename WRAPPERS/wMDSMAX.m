...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wMDSMAX(prob, func, options)
%
% wrapper for the MDSMAX solver
%
%
% ARGUMENTS
% ---------
...
%
%     .simplex_form (int): 1
%       determines the form of the initial simplex
%       0: regular simplex (sides of equal length) 1: right-angled
%       simplex
%
%     .muCoeff1 (float): 2.0
%       Coefficient1 for expansion factor mu = coeff1+coeff2*n
%
%     .muCoeff2 (float): 2.0
%       Coefficient2 for expansion factor mu = coeff1+coeff2*n
%
%     .thetaCoeff1 (float): 0.5
%       Coefficient1 for contraction factor theta = coeff1+coeff2*n
%
%     .thetaCoeff2 (float): 0.5
%       Coefficient2 for contraction factor theta = coeff1+coeff2*n
%
%
% RETURNS
% -------
...
function output = wMDSMAX(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('simplex_form',1, 'muCoeff1',2.0,...
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['MDSMAX',num2str(TuneCase)];
      else, solname = 'MDSMAX';
      end
      ...
      % call solver
      try
        output.solverInfo = mdsmax(@funf, x, stopit, params);
