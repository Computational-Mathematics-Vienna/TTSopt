...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wSSBBO(prob, func, options)
%
% wrapper for the SSBBO solver
%
%
% ARGUMENTS
% ---------
...
function output = wSSBBO(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('DiffMaxChange',0.1, 'DiffMinChange',1e-08,...
    'rho',0.0001, 'sigma',0.9, 'mem',10, 'eps1',1e-08, 'gammaf1',0.5,...
    'gammaf2',2, 'gammaf3',1e-12, 'Deltaangle',1e-13, 'deltaa',1e-08);
  solverParams{2} = struct('mem',5);
  solverParams{3} = struct('mem',10);


  % custom exception
  badCase = 'DFO:BadSolverParameterCase';
  badCaseMsg = '%d is not a valid option case. Must be an integer in [-1, 3].';
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['SSBBO',num2str(TuneCase)];
      else, solname = 'SSBBO';
      end
