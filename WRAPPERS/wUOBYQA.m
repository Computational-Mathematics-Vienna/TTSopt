...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% function output = wUOBYQA(prob, func, options)
%
% wrapper for the UOBYQA solver
%
%
% ARGUMENTS
% ---------
...
%     0 is default for BBO problemsif  ~isempty(strfind(finfo.func,"cutest_obj"))
%
...
function output = wUOBYQA(prob, func, options)
  ...
  solverParams = {};
  solverParams{1} = struct('rhobeg',1.0, 'rhoend',1e-06);
...
'  .funInfo (structure): additional info'...
    ...
    otherwise % all arguments -> calls the solver and returns its output
      ...
      if TuneCase > 0, solname = ['UOBYQA',num2str(TuneCase)];
      else, solname = 'UOBYQA';
      end
      ...
      try
        if isOctave
            ...
            try
              ...
              '/PYTHON/run_uobyqa.py ',xStr,' ',lowStr,...
              ' ',uppStr,' ',paramsStr])
            catch
              fileID = fopen('uobyqaResult.txt','r');
          ...
          [xbest, fbest, ~, ~] = uobyqa(@funf, x, params);
        end
